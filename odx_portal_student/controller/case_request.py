...
class CaseRequestController(CustomerPortal):

    def get_search_domain_case_request(self, search, attrib_values):
        domain = []
        if search:
    ...
    @http.route(['/student/case/request',
                 '/student/case/request<int:student_id>',
                 '/student/case/request<int:student_id>/page/<int:page>',
                 '/student/case/request<int:page>'],
                type='http', auth='user', website=True)
    def all_case_request(
        ...
        domain += self.get_search_domain_case_request(search, attrib_values)
        if student_id:
            keep = QueryURL('/student/case/request/%s' %
                            student_id.id, search=search, amenity=attrib_list,
                            order=post.get('order'))

        else:
            keep = QueryURL('/student/case/request',
        ...
        domain += [('is_mark_review','=',False)]

        student = request.env["op.student"].sudo().search(
            [('user_id', '=', request.env.user.id)])

        total = request.env['grievance'].sudo().search_count([('student_id', '=', student.id),('is_mark_review','=',False)])
        if student_id:
            pager = portal_pager(
                url="/student/case/request/%s" % student_id.id,
        ...
        if groupby == 'state':
            grouped_tasks = [
                request.env['grievance'].sudo().concat(*g).search([('student_id', '=', student_id.id),('is_mark_review','=',False)])
        ...
        if student_id:
            val.update({
                ...
                'filterby': filterby,
                'search_count': total,
                'default_url': '/student/case/request',
                ...
                'groupby': groupby,
                'page_name': 'case_request_form'

            })
            return request.render(
                'odx_portal_student.all_case_request', val)

    @http.route(['/my/case_request/selected_case_request-form/<int:task_id>'],
                type='http', auth='user', website=True)
    def portal_student_case_request_data(self, task_id=None):
        user = request.env.user
        student_id = request.env['op.student'].sudo().search([('user_id', '=', user.id)])
        ...
        return request.render(
            "odx_portal_student."
            "portal_student_case_request_data",
            {'case_request_id': case_request_id,
             'page_name': 'case_request_form',
             'student': student_id,
             })

    @http.route(['/my/case_request/case_request-form'],
                auth='user', website=True, type='http')
    def all_case_request_form(self):
        user = request.env.user
        student_id = request.env['op.student'].sudo().search([('user_id', '=', user.id)])
        ...
        return request.render(
            'odx_portal_student.case_request_form',
            {
    ...
    @http.route(['/my/case_request/case_request-form/submit'], type='http', auth="public", website=True)
    def create_case_request_form(self, **kw):
        ...
        create_case_request = request.env['grievance'].sudo().create({
            "student_id": int(name),
            ...
            "is_mark_review":False,
            "is_academic":True
        })
        create_case_request.submitted_progressbar()
        return werkzeug.utils.redirect('/my/case_request/selected_case_request-form/%s'%create_case_request.id)
